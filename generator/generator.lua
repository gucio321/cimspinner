...
--------------------------header definitions
local cimgui_header = 
[[//This file is automatically generated by generator.lua from https://github.com/gucio321/cimspinner
//based on imspinner.h file version XXX from imspinner https://github.com/dalerank/imspinner
]]
...
local function cimgui_generation(parser,name)
    ...
    --merge it in cimgui_internal_template.cpp to cimspinner.cpp
...
-----------------------------do it----------------------
--------------------------------------------------------
--get imspinner.h version--------------------------
local pipe,err = io.open("../imspinner/imspinner.h","r")
if not pipe then
    error("could not open file:"..err)
end
--local imspinner_version
--while true do
    --local line = pipe:read"*l"
    --imspinner_version = line:match([[ImPlot%s+v(.+)]])
    --if imspinner_version then break end
--end
pipe:close()
--cimgui_header = cimgui_header:gsub("XXX",imspinner_version)
--print("IMPLOT_VERSION",imspinner_version)
...
-------------funtion for parsing imspinner headers
local function parseImGuiHeader(header,names)
	...
	parser.getCname = function(stname,funcname,namespace)
		if stname == funcname then
			return stname .. "_" .. funcname
		end
...
local headers = [[#include "../imspinner/imspinner.h" 
]]
local headersT = {[[imspinner]]}
if INTERNAL_GENERATION then
